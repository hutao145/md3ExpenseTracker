...
| 🎨 个性化主题 | 樱花粉、海湾蓝等 5 套自定义精美色彩预设，完美适配 Material You 动态取色 |
| 🌙 AMOLED 纯黑反馈 | 系统夜间模式下可选极致纯黑背景，更省电且深邃 |
| ⚡ 一触直达筛选 | 点击主页分类统计明细，瞬间穿透筛选查阅列表 |
| ✍️ 全开放自定义类目 | 当内置收支类不够用时，支持自如填入任意专属类别名称 |
| 🤚 顺滑抽屉编辑 | 废弃传统粗暴弹窗，重构为触手可及的单手底部滑出抽屉 |
| 🚀 悬浮回顶指令 | 智能吸附与隐匿的长列表快速返回顶部辅助体验 |
